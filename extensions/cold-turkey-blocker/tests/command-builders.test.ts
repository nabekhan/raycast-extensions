...
test("builds every supported start form", () => {
  ...
  assert.deepEqual(buildStartArgs({ blockName: "Deep Work", mode: "password", password: "focus123" }), [
    "-start",
    "Deep Work",
    "-password",
    "focus123",
  ]);
  assert.deepEqual(buildStartArgs({ blockName: "Deep Work", mode: "random-text", randomTextLength: 120 }), [
    "-start",
    "Deep Work",
    "-random-text",
    "120",
...
test("builds stop, toggle, status, and add-entry forms", () => {
  assert.deepEqual(buildStopArgs("Deep Work"), ["-stop", "Deep Work"]);
  assert.deepEqual(buildStopArgs("Deep Work", "focus123"), ["-stop", "Deep Work", "-password", "focus123"]);
  assert.deepEqual(buildToggleArgs("Deep Work"), ["-toggle", "Deep Work"]);
...
test("rejects invalid numeric and password parameters", () => {
  ...
  assert.throws(
    () => buildStartArgs({ blockName: "Deep Work", mode: "random-text", randomTextLength: 1000 }),
    /between 1 and 999/,
  );
  assert.throws(
    () => buildStartArgs({ blockName: "Deep Work", mode: "password", password: "two words" }),
    /spaces or quote/,
  );
});
